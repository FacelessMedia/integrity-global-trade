/**
 * Chatbot Knowledgebase
 * Compiled from all website content — serves as the system context for the AI chatbot.
 * This is sent as the system prompt to OpenAI so the chatbot can answer questions
 * about IGTC using real business information from the website.
 */

export const KNOWLEDGEBASE = `
# Integrity Global Trade & Commodities Corp (IGTC) — Complete Business Knowledgebase

## Company Overview
Integrity Global Trade & Commodities Corp (IGTC) is a premier global commodities trading company specializing in precious metals, non-ferrous metals, and critical minerals for semiconductor manufacturing and industrial applications. Founded in 2022 by Timothy Mercer, the company has allocated over $1 billion in contract volume, operates in 50+ countries, and maintains a 100% KYC/AML compliance rate.

**Headquarters:** 30 N. Gould St. STE R, Sheridan, Wyoming 82801, USA
**Phone:** +1 (773) 219-7674
**Email:** tim@integritygtc.com
**Website:** https://www.integritygtc.com
**Founder & CEO:** Timothy Mercer
**LinkedIn:** https://www.linkedin.com/in/timothy-mercer-36197977/
**Established:** 2022
**Years in Business:** 4+

## Key Statistics
- $1B+ in Allocated Contract Volume
- 4+ Years in Operation
- 100% KYC/AML Compliance Rate
- 50+ Countries Served
- UN-Certified Mine Sourcing
- LBMA & OECD Standards Adherence

## About Timothy Mercer (Founder & CEO)
Timothy Mercer is the Founder and CEO of Integrity Global Trade & Commodities Corp. He leads all trading operations and client relationships personally. Tim brings deep expertise in global commodities markets, precious metals trading, and compliance-first business practices. He is committed to ethical sourcing and maintaining the highest standards of transparency in every transaction.

## Services

### 1. Precious Metals Trading
Global sourcing and trading of gold, silver, platinum, and palladium from UN-certified mines with full chain-of-custody documentation.
- Physical delivery of London Good Delivery bars
- Spot and forward contract execution
- Full traceability from mine to market
- LBMA and IPMR accredited refinery partnerships

### 2. Non-Ferrous Metals
Trading and supply of copper, aluminium, zinc, nickel, and other non-ferrous metals essential for modern manufacturing and infrastructure.
- Superior electrical conductivity metals
- LME-grade material sourcing
- Industrial-scale supply capability
- Custom alloy specifications

### 3. Critical Minerals & Rare Earths
Strategic sourcing of critical minerals and rare earth elements vital for the energy transition and advanced technology manufacturing.
- Lithium, cobalt, manganese sourcing
- Rare earth element procurement
- Energy transition material supply
- Conflict-free certified sourcing

### 4. Semiconductor Metals
Specialized precious metals refining and supply for semiconductor chip wafer coating, electronic connections, and advanced microelectronics.
- Ultra-high purity gold for chip wafer coating
- Silver for electronic interconnections
- Platinum group metals for catalytic applications
- Semiconductor-grade quality certification

### 5. Refining & Processing
Partnership with IPMR and world-class refineries for state-of-the-art Miller Processing and precious metals refining to the highest purity standards.
- IPMR partnership for Miller Processing
- LBMA Good Delivery accreditation
- Multi-metal refining capabilities
- Assay and quality certification

### 6. Supply Chain Management
End-to-end metals supply chain management from certified mine sourcing through refining, logistics, and final delivery.
- Mine-to-market chain of custody
- Secure logistics and transportation
- Bonded warehouse storage
- Real-time shipment tracking

### 7. Risk Management & Hedging
Comprehensive commodity price risk management solutions including hedging strategies, forward contracts, and market intelligence.
- Price hedging strategies
- Forward and futures contract execution
- Market analysis and intelligence
- Portfolio risk assessment

## Commodities We Trade

### Gold (Au)
The world's most trusted precious metal. IGTC sources and trades physical gold from certified mines for investment, industrial, and semiconductor applications.
Applications: Semiconductor chip wafer coating, Electronic connections, Investment bars and coins, Jewelry manufacturing, Aerospace components.

### Silver (Ag)
The most electrically conductive metal. Essential for electronics, solar energy, and industrial applications with growing demand in green technology.
Applications: Electronics and circuitry, Solar panel manufacturing, Medical devices, Industrial catalysts, Photography and imaging.

### Platinum & Palladium (Pt/Pd)
Platinum group metals critical for catalytic, automotive, and high-technology applications with unique chemical and physical properties.
Applications: Automotive catalytic converters, Fuel cell technology, Chemical processing catalysts, Medical implants, Electronics.

### Copper (Cu)
The premier non-ferrous conductor metal. Essential for electrical infrastructure, renewable energy, and the global electrification transition.
Applications: Electrical wiring and infrastructure, Renewable energy systems, Electric vehicle components, Construction, Telecommunications.

### Critical Minerals (Li/Co/Mn)
Strategic minerals essential for battery technology, renewable energy, and advanced manufacturing including lithium, cobalt, and manganese.
Applications: Battery technology (EV & storage), Renewable energy infrastructure, Advanced electronics, Defense applications, Medical technology.

## Industries Served

### Semiconductor & Electronics
Ultra-high purity precious metals for chip wafer coating, die bonding, wire bonding, and advanced electronic interconnections. IGTC supplies gold, silver, platinum, palladium, and copper for this sector.

### Automotive
Platinum group metals for catalytic converters, copper for EV wiring and motors, and critical minerals for battery packs in next-generation vehicles.

### Aerospace & Defense
High-purity precious metals and specialized alloys for aircraft components, satellite systems, defense electronics, and propulsion systems.

### Jewelry & Luxury
LBMA-certified gold, silver, and platinum for fine jewelry, luxury watches, and high-end consumer goods manufacturing.

### Industrial Manufacturing
Non-ferrous metals and specialized alloys for heavy machinery, tooling, chemical processing, and industrial infrastructure.

### Renewable Energy
Copper for wind turbines and solar installations, silver for photovoltaic cells, and critical minerals for energy storage systems.

## Compliance & Ethics

IGTC operates with a compliance-first approach. Every transaction undergoes rigorous verification.

### KYC/AML Compliance
Every corporate customer must fully comply with Know Your Customer requirements and consent to comprehensive background screening, regardless of size.

### AI-Powered Sanctions Screening
IGTC utilizes ComplyAdvantage AI software to ensure full compliance with government sanctions, ITG and banking policies in real-time.

### UN Certified Mine Sourcing
All raw materials are sourced exclusively from industrial mines holding the highest standards for labor practices and environmental stewardship.

### Environmental Impact Certification
The supply chain exclusively sources from Environmental Impact Certified mines committed to sustainable and responsible mining practices.

### LBMA & OECD Standards
Full adherence to London Bullion Market Association responsible sourcing guidelines and OECD due diligence standards for minerals.

### Chain of Custody Documentation
Complete mine-to-market traceability with documented chain of custody for every shipment, ensuring full transparency and accountability.

### Due Diligence Process
IGTC's 5-step due diligence process:
1. Initial Assessment — Evaluate counterparty identity, ownership structure, and business legitimacy
2. Enhanced KYC — Collect and verify beneficial ownership documents, government IDs, and corporate registrations
3. Sanctions & PEP Screening — Run all parties through ComplyAdvantage AI for real-time sanctions, PEP, and adverse media checks
4. Source Verification — Verify mine certifications, chain of custody documents, and environmental impact assessments
5. Ongoing Monitoring — Continuous monitoring of all counterparties with automated re-screening at regular intervals

### Sanctions Screening
IGTC screens against: UN Consolidated Sanctions List, US OFAC (SDN List), EU Consolidated Sanctions, UK HM Treasury Sanctions, FATF High-Risk Jurisdictions, and Politically Exposed Persons (PEP) databases.

### Chain of Custody
Every shipment includes: Mine origin certificate, Assay reports and purity certification, Transport and handling documentation, Refinery processing records, Final delivery confirmation, and Digital chain of custody record.

## How We Work (5-Step Process)
1. Initial Consultation — Understanding requirements, volumes, specifications, and delivery timelines
2. Due Diligence & KYC — Comprehensive background screening and compliance verification
3. Sourcing & Procurement — Sourcing from certified mines and verified suppliers worldwide
4. Refining & Quality Assurance — Processing through LBMA-accredited refineries with quality certification
5. Secure Delivery — Bonded logistics, insured transport, and documented chain of custody to destination

## Why Choose IGTC
- Compliance-first approach with 100% KYC/AML on every transaction
- $1B+ proven track record in allocated contract volume
- Direct mine-to-market relationships eliminating intermediary costs
- LBMA & OECD standard adherence
- 50+ country global network across 6 continents
- AI-powered sanctions screening via ComplyAdvantage
- Full chain of custody documentation on every shipment
- Semiconductor-grade purity metals
- Dedicated account management with founder-led service

## Contact & Consultation
To discuss trading requirements, request a consultation, or learn more:
- Email: tim@integritygtc.com
- Phone: +1 (773) 219-7674
- Website contact form: https://www.integritygtc.com/contact
- LinkedIn: https://www.linkedin.com/in/timothy-mercer-36197977/

IGTC offers free initial consultations to discuss how they can serve prospective clients' commodities needs.
`;
